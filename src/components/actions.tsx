    ...
    <div>
      ...
      {categoryEntries.map(([category, actionsForCategory], index) => (
        <CategoryComponent
          ...
          key={category}
          isFirst={firstCategoryIsFirst && index === 0}
        />
      ))}
    </div>
  );
...
export const CategoryComponent = ({
  ...
  category: string;
  actionsForCategory: Action[];
  isFirst?: boolean;
}) => {
  const { openedCategory, setOpenedCategory, focusedElement, setFocusedElement } =
    useMenuContext();
  const { ref } = useFocusElement(focusedElement, `category:${category}`, isFirst);

  const onFocus = useCallback(
    () => setFocusedElement(`category:${category}`),
  ...
  return (
    <div key={category} ref={ref}>
      <ActionButton
        ...
        icon={
          <img
            src={getIconPath(`RD-zoom-${openedCategory === category ? "out" : "in"}`)}
            width={24}
            height={24}
          />
        }
...
export const ActionComponent = ({
  ...
  const { ref, isFocused } = useFocusElement(
    focusedElement,
    `action:${action.id}`,
    isFirst
  );
  ...
  const onFocus = useCallback(
    () => setFocusedElement(`action:${action.id}`),
    [setFocusedElement, action.id]
  );
  ...
  const icon =
    action.icon.type === "path" ? (
      <img
        alt={action.name}
        src={getIconPath(action.icon.value)}
        width={24}
        height={24}
      />
    ) : (
...
const useFocusElement = (
  ...
  elementId: string,
  scrollToTop?: boolean
): { ref: RefObject<HTMLDivElement | null>; isFocused: boolean } => {
