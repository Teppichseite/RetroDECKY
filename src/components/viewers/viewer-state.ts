...
export function useCachedPdfViewState(
  pdfPath: string
): [PdfViewState, Dispatch<SetStateAction<PdfViewState>>] {
  const cacheKey = `pdf:${normalizePath(pdfPath)}`;
  const cacheKeyRef = useRef(cacheKey);
  cacheKeyRef.current = cacheKey;

  const [viewState, setViewStateInternal] = useState<PdfViewState>(() =>
    loadPdfViewState(cacheKey)
  );
  ...
  const setViewState = useCallback(
    (value: SetStateAction<PdfViewState>) => {
...
export function useCachedTextViewState(
  txtPath: string
): [
  number,
  ...
  const [state, setStateInternal] = useState<TextViewState>(() =>
    loadTextViewState(cacheKey)
  );
