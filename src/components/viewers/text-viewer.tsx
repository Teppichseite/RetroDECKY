...
export const TextViewer = ({ txtPath, title }: TextViewerProps) => {
  ...
  const [fontSize, setFontSize, scrollTop, setScrollTop] =
    useCachedTextViewState(txtPath);
  const scrollContainerRef = useRef<HTMLDivElement>(null);

  const containerHeight = useMemo(() => Math.floor(findSP().innerHeight * 0.7), []);
  const containerWidth = useMemo(() => Math.floor(findSP().innerWidth * 0.7), []);
